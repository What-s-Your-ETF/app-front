...
function ETFss() {
  ...
  const CustomizedDot = (props) => {
    ...
    if (value > 2500) {
      return (
        <svg x={cx} y={cy} width={2} height={2} fill="red" viewBox="0 0 1024 1024">
    ...
    return (
      <svg x={cx} y={cy} width={2} height={2} fill="green" viewBox="0 0 1024 1024">
  ...
  const getGraph = (data)=>{
    ...
    <LineChart width={730} height={350} data={data} onClick={handleDataClick} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
        ...
        {keys.map((elem,idx)=>{
            if(idx>=1)
                return (<Line type="monotone" dataKey={elem} stroke="#82ca9d" dot={<CustomizedDot />} />)
        })}
  ...
  const PieChart2 = useMemo(() =>{
    ...
    <PieChart width={730} height={250} >
        ...
        {data01.map((entry, index) => (
            <Cell key={`cell-${index}`} fill={COLORS[Math.round(Math.random()*19)]} />
          ))}
        </Pie>
    </PieChart>
    )
